...
// 【隨機突襲：飛升試煉】遭遇戰狀態
let trialQuestion = "癮癮端著買一送一的超肥美芋圓鮮奶茶誘惑你，今日『想』已無額度！";
let trialAnserA = "理智斬妖：改喝免費水靈(靈力+5)";
let trialAnserB = "沉淪魔道：買啦！借下個月額度(種下心魔)";
...
let needMonsters = [
    { label: "米寶", unlocked: true, waveCount: 3, hint: "" }, { label: "水靈", unlocked: false, waveCount: 0, hint: "未知的心魔尚未甦醒... 需連續 3 天記帳『醫』類別。" },
    { label: "布丁犬", unlocked: true, waveCount: 2, hint: "" }, { label: "草靈", unlocked: true, waveCount: 1, hint: "" },
    { label: "守護貓", unlocked: false, waveCount: 0, hint: "未知的心魔尚未甦醒... 需在『住』維持理性消費 5 次。" }, { label: "光精靈", unlocked: false, waveCount: 0, hint: "未知的心魔尚未甦醒... 需存滿 10000 元資產解鎖。" },
    { label: "鐵匠米寶", unlocked: false, waveCount: 0, hint: "未知的心魔尚未甦醒... 需觸發累積滿 10 次『需』審判。" }, { label: "盾御獸", unlocked: true, waveCount: 2, hint: "" },
    { label: "小飛俠", unlocked: false, waveCount: 0, hint: "未知的心魔尚未甦醒... 需解鎖全部『需』字生靈。" }
];
let wantMonsters = [
    { label: "癮癮", unlocked: true, waveCount: 3, hint: "" }, { label: "饞饞", unlocked: false, waveCount: 0, hint: "未知的心魔尚未甦醒... 需在『飲』類別放縱消費 3 次以引誘其現身。" },
    { label: "剁手怪", unlocked: true, waveCount: 2, hint: "" }, { label: "買買妖", unlocked: true, waveCount: 1, hint: "" }, { label: "夜貓鬼", unlocked: true, waveCount: 3, hint: "" },
    { label: "課金蟲", unlocked: false, waveCount: 0, hint: "未知的心魔尚未甦醒... 需在『3C』放縱破戒一次。" }, { label: "爆食魔", unlocked: false, waveCount: 0, hint: "未知的心魔尚未甦醒... 需單筆消費『食』類別超過 1000 元。" },
    { label: "慵懶怪", unlocked: false, waveCount: 0, hint: "未知的心魔尚未甦醒... 需在『交』類別超預算破戒。" }, { label: "小惡魔", unlocked: true, waveCount: 2, hint: "" }
];
...
function initBalanceEcosystem() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        balanceEcosystem.push({
            label: categories[i], color: pastelColors[i], angle: angle, budget: 100, currentR: radius,
...
function drawCommonUI() {
    ...
    // 【全新重構：穿心繞環因果鎖鏈線】
    // 一條線連結在五個圓圈的中間，遇到圓圈就會往下繞圓圈的下緣繼續連結下一個圓圈。
...
function renderLedgerPage() {
    ...
    drawCuteLine(abX, abY, abX + abW, abY, 2); drawCuteLine(abX + abW, abY, abX + abW, abY + abH, 2);
    drawCuteLine(abX + abW, abY + abH, abX, abY + abH, 2); drawCuteLine(abX, abY + abH, abX, abY, 2);
    ...
    if (appState === 'MONTH_AWAKE') {
        ...
        ctx.fillStyle = '#EAEAEA'; ctx.fillRect(140, 380, 120, 32); drawCuteLine(140,380,260,380); drawCuteLine(260,380,260,412); drawCuteLine(260,412,140,412); drawCuteLine(140,412,140,380);
        ctx.fillStyle = '#4A4A4A'; ctx.font = 'bold 13px "Microsoft JhengHei", sans-serif';
        ctx.fillText(awakeReportStatus === 'WIN' ? "覺醒解鎖" : "簽下悔過書", 165, 401);
    }
}
...
function renderGalleryPage() {
    let list = (currentTab === '需') ? needMonsters : wantMonsters; let tcy = 115;
    ctx.fillStyle = (currentTab === '需') ? '#A8E6CF' : '#EAEAEA'; ctx.beginPath(); ctx.moveTo(100, tcy); ctx.quadraticCurveTo(145, tcy - 35, 190, tcy); ctx.closePath(); ctx.fill(); drawCuteLine(100, tcy, 190, tcy, 2); ctx.beginPath(); ctx.moveTo(100, tcy); ctx.quadraticCurveTo(145, tcy - 35, 190, tcy); ctx.stroke();
    ctx.fillStyle = (currentTab === '想') ? '#FF8B94' : '#EAEAEA'; ctx.beginPath(); ctx.moveTo(210, tcy); ctx.quadraticCurveTo(255, tcy - 35, 300, tcy); ctx.closePath(); ctx.fill(); drawCuteLine(210, tcy, 300, tcy, 2); ctx.beginPath(); ctx.moveTo(210, tcy); ctx.quadraticCurveTo(255, tcy - 35, 300, tcy); ctx.stroke();
    ctx.fillStyle = '#4A4A4A'; ctx.font = 'bold 16px "Microsoft JhengHei", sans-serif'; ctx.fillText("需", 137, tcy - 8); ctx.fillText("想", 247, tcy - 8); drawCuteLine(30, tcy, 370, tcy, 3);

    let startX = 75, startY = 175, gap = 125; if (shockTimer > 0) shockTimer--;
    for (let i = 0; i < 9; i++) {
        let row = Math.floor(i / 3), col = i % 3; let cellX = startX + col * gap, cellY = startY + row * gap; let monster = list[i];
        let offsetX = 0, offsetY = 0; if (shockMonsterIndex === i && shockTimer > 0) { offsetX = (Math.random() - 0.5) * 6; offsetY = (Math.random() - 0.5) * 6; }
        let finalCX = cellX + offsetX, finalCY = cellY + offsetY, r = 38;
        ctx.fillStyle = monster.unlocked ? '#E2F0CB' : '#F3F3F3'; ctx.beginPath(); ctx.arc(finalCX, finalCY, r, 0, Math.PI*2); ctx.fill(); drawCuteCircle(finalCX, finalCY, r, 3, '#4A4A4A');
        if (monster.unlocked) {
            ctx.save(); ctx.beginPath(); ctx.arc(finalCX, finalCY, r, 0, Math.PI*2); ctx.clip();
            drawCuteLine(finalCX - 12, finalCY - r, finalCX - 12, finalCY + r, 1.5, 'rgba(74,74,74,0.4)'); drawCuteLine(finalCX, finalCY - r, finalCX, finalCY + r, 2, 'rgba(74,74,74,0.5)'); drawCuteLine(finalCX + 12, finalCY - r, finalCX + 12, finalCY + r, 1.5, 'rgba(74,74,74,0.4)');
            ctx.strokeStyle = '#4A4A4A'; ctx.lineWidth = 1.5;
            for (let w = 0; w < monster.waveCount; w++) {
                let wy = finalCY + 12 + (w * 6); ctx.beginPath();
                for (let wx = finalCX - r; wx <= finalCX + r; wx += 5) { let waveY = wy + Math.sin(wx * 0.15 + waveOffset) * 2; if (wx === finalCX - r) ctx.moveTo(wx, waveY); else ctx.lineTo(wx, waveY); }
                ctx.stroke();
            }
            ctx.restore(); ctx.fillStyle = '#4A4A4A'; ctx.font = 'bold 13px "Microsoft JhengHei", sans-serif'; ctx.fillText(monster.label, finalCX - ctx.measureText(monster.label).width/2, finalCY + 5);
        } else {
            ctx.fillStyle = 'rgba(74,74,74,0.3)';
            if (i % 2 === 0) { ctx.font = 'bold 22px "Comic Sans MS", sans-serif'; ctx.fillText("?", finalCX - 6, finalCY + 8); }
            else { let lockX = finalCX + 14, lockY = finalCY + 14; ctx.fillStyle = '#FFFFFF'; ctx.fillRect(lockX, lockY, 12, 10); ctx.strokeStyle = '#4A4A4A'; ctx.lineWidth = 1.5; ctx.strokeRect(lockX, lockY, 12, 10); ctx.beginPath(); ctx.arc(lockX + 6, lockY, 4, Math.PI, 0); ctx.stroke(); }
        }
    }
}
...
function renderPreceptPage() {
    ...
    if (sub结Mode === '平衡儀') {
        ...
        balanceEcosystem.forEach((item, i) => {
            item.currentR = 45 + (item.budget / 300) * 110; item.x = cx + Math.cos(item.angle) * item.currentR; item.y = cy + Math.sin(item.angle) * item.currentR;
            drawCuteLine(cx, cy, item.x, item.y, 1.5, 'rgba(74, 74, 74, 0.35)');
            ctx.fillStyle = item.color; ctx.beginPath(); ctx.arc(item.x, item.y, 20, 0, Math.PI*2); ctx.fill();
            ...
            drawCuteCircle(item.x, item.y, 20, borderThickness, borderColor);
...
function renderSocialPage() {
    ...
    drawCuteLine(boardX + shakeX, boardY, boardX + boardW + shakeX, boardY, 3); drawCuteLine(boardX + boardW + shakeX, boardY, boardX + boardW + shakeX, boardY + boardH, 3); drawCuteLine(boardX + boardW + shakeX, boardY + boardH, boardX + shakeX, boardY + boardH, 3); drawCuteLine(boardX + shakeX, boardY + boardH, boardX + shakeX, boardY, 3);
    ...
    if (socialTab === '動態') {
        ctx.font = 'bold 15px "Microsoft JhengHei", sans-serif'; ctx.fillText("📜 驅魔動態牆 (公會即時因果)", boardX + 32, boardY + 40);
        ctx.font = '12px "Microsoft JhengHei", sans-serif';
        guildFeeds.forEach((feed, idx) => { let fy = boardY + 74 + idx * 28; ctx.fillStyle = (feed.style === 'curse') ? '#FF5252' : '#4A4A4A'; ctx.fillText(feed.text, boardX + 32, fy); });
    } else {
        ctx.font = 'bold 15px "Microsoft JhengHei", sans-serif'; ctx.fillText("🏆 全體好友修行排行榜", boardX + 32, boardY + 40);
        ctx.font = '12px "Microsoft JhengHei", sans-serif';
        friendRankings.forEach((f, idx) => { let fy = boardY + 72 + idx * 26; ctx.fillStyle = '#4A4A4A'; ctx.fillText(`${f.rank}. ${f.icon} ${f.name}`, boardX + 32, fy); ctx.fillStyle = '#4E9F3D'; ctx.fillText("生存 " + f.days + " 天", boardX + boardW - 105, fy); });
    }
    ctx.restore();
    ...
    drawCuteLine(px, py - 40, px + 4, py + 95, 4, '#D7C49E');
    ctx.fillStyle = '#CDA45E'; ctx.fillRect(px - 45, py - 30, 90, 26);
    drawCuteLine(px - 45, py - 30, px + 45, py - 30, 2.5); drawCuteLine(px + 45, py - 30, px + 45, py - 4, 2.5); drawCuteLine(px + 45, py - 4, px - 45, py - 4, 2.5); drawCuteLine(px - 45, py - 4, px - 45, py - 30, 2.5);
    ctx.fillStyle = '#4A4A4A'; ctx.font = 'bold 14px "Microsoft JhengHei", sans-serif'; ctx.fillText("排行榜", px - 21, py - 12);

    if (ghostHideTimer === 0) {
        let ghostFloat = Math.sin(Date.now() * 0.005) * 2.5; 
        ctx.save(); ctx.fillStyle = '#FFFFFF'; ctx.globalAlpha = 0.92; ctx.beginPath(); ctx.arc(px - 32, py - 34 + ghostFloat, 9, Math.PI, 0, false); ctx.lineTo(px - 23, py - 29 + ghostFloat); ctx.lineTo(px - 41, py - 29 + ghostFloat); ctx.closePath(); ctx.fill();
        ctx.fillStyle = '#000000'; ctx.beginPath(); ctx.arc(px - 35, py - 36 + ghostFloat, 1.5, 0, Math.PI*2); ctx.arc(px - 29, py - 36 + ghostFloat, 1.5, 0, Math.PI*2); ctx.fill(); ctx.restore();
    }

    let branchY = py + 8; drawCuteLine(px - 42, branchY, px + 42, branchY, 2); 
    ctx.save(); ctx.translate(px, branchY); ctx.rotate(tagHangingShock * Math.PI / 180); 
...
function renderTrialBattlefield() {
    ...
    ctx.fillStyle = '#4A4A4A'; ctx.font = '12px "Microsoft JhengHei", sans-serif'; ctx.fillText(trialQuestion.substring(0, 25), bx + 20, by + 75); ctx.fillText(trialQuestion.substring(25), bx + 20, by + 100);
    ...
    ctx.font = '12px "Microsoft JhengHei", sans-serif'; ctx.fillText(trialAnserA, ax + 35, ay + 5);
    ...
    ctx.fillStyle = '#4A4A4A'; ctx.font = '12px "Microsoft JhengHei", sans-serif'; ctx.fillText(trialAnserB, bx2 + 35, by2 + 5);
...
canvas.addEventListener('mousedown', function(e) {
    ...
    if (currentPage === '結' && sub结Mode === '平衡儀' && appState === 'MAIN') {
        for (let i = 0; i < 10; i++) {
            let item = balanceEcosystem[i]; if (Math.hypot(mx - item.x, my - item.y) < 22) { draggedCircleIndex = i; break; }
        }
    }
});
...
canvas.addEventListener('mousemove', function(e) {
    if (draggedCircleIndex !== -1 && currentPage === '結' && sub结Mode === '平衡儀' && appState === 'MAIN') {
        ...
        let targetBudget = Math.round((dist - 45) / 110 * 300);
...
canvas.addEventListener('click', function(e) {
    ...
    if (appState === 'TRIAL') {
        ...
        if (Math.hypot(mx - ax, my - ay) < ar) { trialFeedbackText = "⚡ 降下天雷把癮癮劈飛！內功靈力+5，信心大增！"; trialFeedbackTimer = 120; return; }
        if (Math.hypot(mx - bx2, my - by2) < ar) { trialFeedbackText = "🦊 癮癮長大三倍發出大笑！帳簿被塞入心魔種子！"; trialFeedbackTimer = 120; return; }
        return;
    }

    if (appState === 'MONTH_AWAKE' && currentPage === '帳') {
        if (mx > 140 && mx < 260 && my > 380 && my < 412) {
    ...
    navButtons.forEach(btn => {
        if (Math.hypot(mx - btn.x, my - btn.y) < btn.r) {
            if (['修', '圖', '帳', '結', '社'].includes(btn.label)) {
                ...
                if (currentPage === '修' && Math.random() < 0.25) { appState = 'TRIAL'; }
            }
        }
    });
    ...
    if (currentPage === '社') {
        let boardX = 40, boardY = 110, boardW = 320, boardH = 195;
        if (mx > 40 && mx < 130 && my > 320 && my < 460) { boardShockTimer = 15; ghostHideTimer = 90; tagHangingShock = 35; socialTab = (socialTab === '動態') ? '排行榜' : '動態'; return; }
        if (mx > boardX + 12 && mx < boardX + boardW - 12 && my > boardY + 12 && my < boardY + boardH - 12) { boardShockTimer = 20; alert("🔮【因果窺探鏡】對方生存剩餘 14 天 ｜ 主神靈健康無恙。"); return; }
    }
    ...
    if (currentPage === '帳') {
        ...
        if (dropdownOpen) {
            let dy = 334 + 6; let dropW = 120; let dropX = 45 + 75;
    ...
    if (currentPage === '圖') {
        let tcy = 115;
        if (mx > 100 && mx < 190 && my > tcy - 35 && my < tcy) { currentTab = '需'; infoAlertText = ""; infoAlertTimer = 0; return; }
        if (mx > 210 && mx < 300 && my > tcy - 35 && my < tcy) { currentTab = '想'; infoAlertText = ""; infoAlertTimer = 0; return; }
        let startX = 75, startY = 175, gap = 125; let list = (currentTab === '需') ? needMonsters : wantMonsters;
        for (let i = 0; i < 9; i++) {
            let row = Math.floor(i / 3), col = i % 3; let cellX = startX + col * gap, cellY = startY + row * gap;
            if (Math.hypot(mx - cellX, my - cellY) < 38) {
                let monster = list[i];
                if (monster.unlocked) { shockMonsterIndex = i; shockTimer = 25; setTimeout(() => { alert(`【因果修煉簿】${monster.label}`); }, 100); }
                else { infoAlertText = monster.hint; infoAlertTimer = 180; }
                break;
            }
        }
    }
